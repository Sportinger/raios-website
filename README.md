...
3. Der Button bleibt während seiner Einführung räumlich vollständig statisch und von Anfang an opak; nur reales Licht macht ihn sichtbar. Eine unsichtbare neutralweiße Punktlichtquelle hält zunächst eindeutig hinter seiner Rückseite und orbitiert anschließend auf einer horizontalen 240-Grad-Kreisbahn gegen den Uhrzeigersinn nach vorne. Während dieses Reveals sind globales Hemisphere-/Key-Light und das frühere blaue Button-Point-Light deaktiviert; das orbitierende Point Light ist die einzige reale Lichtquelle und erzeugt über eine `BasicShadowMap` bewusst harte wandernde Schatten. Auch das Power-Symbol verwendet ein graues `MeshStandardMaterial` und wird erst beim Einschalten blau-emissiv. Erst nachdem der Lichtorbit vollständig beendet ist, umkreist der Camera-Rig den Button um 90 Grad nach rechts. Exakt beim vollständig gedrückten Zustand starten ON-Impuls und Kamera-Follow gemeinsam. Der Impulskern bleibt als exaktes Kameraziel im Bildzentrum, während der Follow zunächst kräftig beschleunigt, anschließend ruhiger wird, kontinuierlich an Höhe und Abstand gewinnt und auf die ursprüngliche Kameraseite zurückorbitiert. Das globale Licht fährt erst später für Bare Metal und Kernel hoch.
