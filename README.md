...
```text
features/
├── hardware-platform/   Sechs Hardwareblöcke, Aktivierungsfolge und Busse
...
4. `chapters/boot-sequence/` übernimmt anschließend als zusammenhängender Objektgraph und veröffentlicht acht navigierbare Abschnitte. Auf der unbeschrifteten Bare-Metal-Platte liegen sechs unterschiedlich proportionierte Blöcke: CPU, `UEFI · SPI FLASH`, Chipsatz mit Speichercontroller, RAM, `USB · xHCI` und andere Geräte. Eine flächige Lichtwelle enthüllt sie zunächst nur als dunkle Silhouetten. Danach leuchten CPU und SPI-Flash auf, ein vorhandener `data-stream` visualisiert die Firmwareausführung, Chipsatz und RAM werden mit kurzen Memory-Training-Pulsen aktiviert und erst anschließend erscheinen die wiederverwendbaren `circuit-trace`-Busse. Der xHCI-Controller wird zuletzt aktiv; der folgende USB-Abschnitt visualisiert Erkennung und Lesen des Sticks, während andere Geräte bis zur späteren Kernelübernahme dunkel bleiben. Danach entstehen Limine, Kernel, Startinformationen und die gerichtete Tür `UEFI → RAIOS` wie bisher.
