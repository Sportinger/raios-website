...
2. Ein von Anfang an vollständig sichtbares, grau-metallisches Kabel beginnt mittig an der Rückseite des Buttons, läuft zunächst gerade aus dem Gehäuse, sinkt auf eine unsichtbare Bodenhöhe ab und schlängelt sich erst dort bis an die Seitenkante des weit entfernten `BARE METAL`-Layers. Ein heller ON-Impuls mit additivem Halo startet bereits im letzten guten Drittel des Kamera-Orbits und läuft sichtbar durch das Kabel. Nur die bereits vom Impuls durchlaufene Strecke wird dauerhaft elektrisch blau und leuchtend; der noch vorausliegende Abschnitt bleibt grau. Button und Layer liegen auf derselben Welt-Höhe; der Layer ist von Anfang an räumlich vorhanden und wird nicht eingeblendet.
3. Der Button bleibt während seiner Einführung räumlich vollständig statisch und von Anfang an opak; nur reales Licht macht ihn sichtbar. Eine unsichtbare neutralweiße Punktlichtquelle hält zunächst eindeutig hinter seiner Rückseite und orbitiert anschließend auf einer horizontalen 240-Grad-Kreisbahn im Uhrzeigersinn nach vorne. Das orbitierende Point Light erzeugt über eine `BasicShadowMap` bewusst harte wandernde Schatten. Das Power-Symbol verwendet zunächst ein graues `MeshStandardMaterial` und blendet in der zweiten Hälfte des Lichtorbits unabhängig auf raiOS-blaues Emissive plus eine breitere additive Glow-Geometrie ein. Eine direkt über der Symbolfläche sitzende blaue Punktlichtquelle beleuchtet dabei Kappe, Rand und nahe Objekte tatsächlich; die additive Geometrie zeichnet nur die sichtbare Aura. Ab exakt diesem Zeitpunkt fahren Hemisphere- und Key-Light langsam per Smootherstep hinzu, sodass Gehäuse und Rand zunehmend lesbarer werden, ohne den wandernden Lichtakzent abrupt zu überdecken. Erst nachdem der Lichtorbit vollständig beendet ist, umkreist der Camera-Rig den Button um 90 Grad nach rechts. Der ON-Impuls startet bereits im letzten guten Drittel dieses Kamera-Orbits. Nach dessen Ende blendet die Kamera ihr Ziel weich vom Button auf den Impuls um und setzt ihre Bewegung leicht verzögert ein, damit weder Blickrichtung noch Abstand springen. Anschließend bleibt der Impulskern als Kameraziel im Bildzentrum, während der Follow kontinuierlich an Höhe und Abstand gewinnt und auf die ursprüngliche Kameraseite zurückorbitiert.
