...
Die wiederverwendbaren Grundbausteine `objects/cards/`, `objects/connections/cable/`, `objects/effects/energy-flow/` und `objects/effects/data-stream/` kennen diese Fachbegriffe nicht. Auch der UEFI-Pfad vom physischen USB-Port zum USB-Boot-Service verwendet das allgemeine Kabel: Nur seine weich verlegte Route liegt im UEFI-Feature. Das Kabel besitzt unabhängige Reveal- und Retract-Grenzen, sodass ein Lichtkopf die Leitung auslegen und ihr Anfang nach einer Haltephase wieder eingezogen werden kann. Die Energieringe verwenden dasselbe sichtbare Fenster. Sprechertexte und Audio sind bewusst nicht Bestandteil der aktuellen Implementierung.
