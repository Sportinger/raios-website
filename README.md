...
Noch bevor der Impuls in die Platte eintritt, löst `cameraRelease` den Kamerablick weich vom Impuls. Der Flug wird langsamer und endet in einer eigenen weiten Seitenpose, in der die vollständige Bare-Metal-Platte bereits den Bildmittelpunkt bildet. Beim Kontakt fährt die Platte sichtbar auf ihre volle Höhe, während das wiederverwendbare `surface-current`-Objekt eine weiche cyanfarbene Energiewelle über ihre Oberfläche schickt. Erst dieser Einschlag startet eine getrennte, bis zum Kapitelende verlangsamte Kamerafahrt in die senkrechte Draufsicht. Kapitel 1 endet damit weit über der aktivierten Hardware, ohne Roll-Flip; die folgende Boot-Sequenz übernimmt dieselbe Orientierung ohne Kamerasprung.
