...
Noch bevor der Impuls in die Platte eintritt, löst `cameraRelease` den Kamerablick weich vom Impuls. Der Flug wird langsamer und endet in einer eigenen weiten Seitenpose, in der die vollständige Bare-Metal-Platte bereits den Bildmittelpunkt bildet. Kurz vor dem sichtbaren Kontakt beginnen sowohl das Hochfahren der Platte als auch die weiche cyanfarbene Energiewelle des wiederverwendbaren `surface-current`-Objekts. Die Kamera bleibt während dieses Aufbaus in ihrer weiten Seitenbewegung und beginnt erst bei lokalem Kapitelwert `0.87` mit der langsamen Drehung in die Draufsicht. Die beiden Kameraabschnitte verwenden komplementäre Momentum-Easings: Der Side-View-Flug kommt mit Restgeschwindigkeit an, die Top-down-Fahrt übernimmt diese Bewegung unmittelbar und bremst erst an ihrem Ende vollständig ab. Kapitel 1 endet damit weit über der aktivierten Hardware, ohne Roll-Flip; die folgende Boot-Sequenz übernimmt dieselbe Orientierung ohne Kamerasprung.
